...
public class PowerBatteryRenderer implements BlockEntityRenderer<PowerBatteryBE> {
    ...
    public static final ResourceLocation POWER_SMOKE = new ResourceLocation(HempFarmer.MODID, "textures/mob_effect/power_smoke");

    public PowerBatteryRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(PowerBatteryBE powerBattery, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {

        Boolean powered = powerBattery.getBlockState().getValue(BlockStateProperties.POWERED);
        if (TRUE != powered) {
            return;
        }
        ...
        float scale = 0.1f + s * (float) (double) PowerBatteryConfig.RENDER_SCALE.get();

        // Get our texture from the atlas
        TextureAtlasSprite sprite = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(POWER_SMOKE);
    ...
    public static void register() {
        BlockEntityRenderers.register(Registration.POWER_BATTERY_BE.get(), PowerBatteryRenderer::new);
    }
